...
int adicionaElemDicionario(dicionario d, void * ch, void * elem){
	...
	auxTup = criaTuplo(d->tipoCh, ch, elem);
	if(auxTup == NULL)
		return 0;

	auxNo = criaNoSimples(auxTup, NULL);
	if(auxNo == NULL)
		return 0;
...
void * removeElemDicionario(dicionario d, void * ch){
	...
	while (auxNo != NULL){
		tuplo tup = elemNoSimples(auxNo);
		if (igualChaves(priTuplo(tup), ch, d->tipoCh) == 1) {
			if (antNo == NULL) {
...
iterador iteradorDicionario(dicionario d){
	...
	for (int i = 0; i < d->tamanho; i++){
		...
		while (auxNo != NULL){
			vector[index] = elemNoSimples(auxNo);
			index++;
			auxNo = segNoSimples(auxNo);
		}
	}
